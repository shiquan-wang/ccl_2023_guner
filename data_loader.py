...
def load_data_bert(config):
    ...
    #with open('./data/{}/test.json'.format(config_0527_1.dataset), 'r', encoding='utf-8') as f:
    #    test_data = json.load(f)

    #bert-base-chinese,guwenbert-base,electra
    #tokenizer = AutoTokenizer.from_pretrained(config_0527_1.bert_name)

    #bert-wwm-ext-base,macbert
...
def fill_newvocab(vocab):
    vocab.add_label('PER')
    vocab.add_label('OFI')
    vocab.add_label('BOOK')


def prepare_predict_input(config, predict_path):
    with open(predict_path, 'r', encoding='utf-8') as f:
        predict_data = json.load(f)
    tokenizer = AutoTokenizer.from_pretrained(config.bert_name)
    #tokenizer = BertTokenizer.from_pretrained(config_0527_1.bert_name)
    vocab = Vocabulary()
    fill_newvocab(vocab)
    config.label_num = len(vocab.label2id)
    config.vocab = vocab
    predict_dataset = RelationDataset(*process_bert(predict_data, tokenizer, vocab))
    return predict_dataset,predict_data